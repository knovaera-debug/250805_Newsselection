...
# =========================
# ワークシート取得（空白トリム付きでシート名解決）
# =========================
def get_worksheet_by_name_trimmed(sheet, target_name):
    for ws in sheet.worksheets():
        if ws.title.strip() == target_name.strip():
            print(f"✅ シート見つかりました: '{ws.title}'")
            return ws
    raise ValueError(f"❌ シート '{target_name}' が見つかりません（空白付き含む）")
...
try:
    ...
    input_ws = get_worksheet_by_name_trimmed(sheet, INPUT_SHEET_NAME)
...
# 出力用のシート作成
try:
    base_ws = get_worksheet_by_name_trimmed(sheet, BASE_SHEET_NAME)
    sheet.duplicate_sheet(source_sheet_id=base_ws.id, new_sheet_name=TODAY_SHEET_NAME)
